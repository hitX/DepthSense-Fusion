...
\
def draw_results(image, depth_map, boxes):
    depth_map = normalize_depth(depth_map)  
    for box in boxes:
        x1, y1, x2, y2, conf, cls = map(int, box)

        box_depths = depth_map[y1:y2, x1:x2]
        if box_depths.size == 0:
            continue

        average_depth = np.mean(box_depths)* 1.66666666667 

        center_x, center_y = (x1 + x2) // 2, (y1 + y2) // 2
        if center_y >= depth_map.shape[0] or center_x >= depth_map.shape[1]:
          continue 
        depth = depth_map[center_y, center_x] * 1.66666666667

        if average_depth > 0:
            width_meters =  (x2-x1) / 384
            
            if average_depth >0.1:
              object_width = width_meters * (average_depth *2)
            else:
              object_width = 0
        else:
           object_width = 0

        label = f"Depth: {depth:.2f}m, Width: {object_width:.2f}m"
